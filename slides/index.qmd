...
## Measuring quality against requirements {.quality-evaluation}

Quality means fitness for the task.
Each requirement needs an observable check.

| What can we check? | How do we assess it? |
|:---|:---|
| Structure, calculations, explicit rules | **Validators and tests** |
| Answers with a known reference | **Reference comparison**: exact match, precision, recall |
| Open-ended work | **Explicit rubric**: correctness, coverage, source support, clarity |
| Subjective or consequential judgments | **Expert review**: compare alternatives against a criterion |
| Evaluation at scale | **LLM judge**, calibrated against human judgments |

::: {.quality-takeaway}

Use the least subjective evaluator that can measure each property.

:::

::: {.notes}

Adapted from our prior discussion of AI output quality, supplied for this talk.
The evaluation chain is task, requirements, quality dimensions, observable criteria, evaluator, metric.
Define the criteria before evaluating the output.

Quality dimensions include correctness, completeness, relevance, groundedness or faithfulness, consistency, instruction adherence, reasoning and application, communication, and safety where applicable.
...
A citation's existence does not establish that it supports the claim.

For a research summary, check whether it represents the major findings, attributes claims to the evidence, preserves uncertainty, and keeps conclusions within what the evidence supports.
...
Check its agreement with human labels and watch for position, verbosity, and self-preference biases.

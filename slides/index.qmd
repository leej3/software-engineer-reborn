...
![](images/llms-timeline-toloka.png){.timeline-figure fig-alt="Timeline of large language model development, from the Turing Test in 1950 through developments in 2025."}
...
- **New mathematics:** OpenAI announced a proposed Navier–Stokes solution with a Lean proof on 8 September.
  Treat this as a newly announced claim pending independent scrutiny.
...
Navier–Stokes: [OpenAI’s announcement](https://openai.com/index/navier-stokes-solution/) describes finite-time singularity formation with smooth forcing, claiming statements C and D of the Millennium Prize formulation.
This is a proposed new mathematical result, unlike Fermat’s formalization of an existing theorem.
OpenAI reports an internal model more capable than Astra, roughly 10,000 concurrent agents in the successful group, 88 hours to the result, and 17 additional hours for Lean formalization and verification.
Attribute the claim to OpenAI rather than presenting independent mathematical acceptance as established.
...
- I'm still clinging to the idea that software engineers become gods
...
- **We turn intent into specifications.** Break a vague goal into parts, constraints, and observable behavior.
- **Git makes experimentation recoverable.** Branch, compare, review, merge, roll back—across documents, code, and agents.
- **Text becomes programmable.** Search, extract, transform, and validate whole collections of files with code.
- **We know how to test a convincing answer.** Check behavior, reproduce failures, and inspect what changed.
- **We turn a useful result into a reusable tool.** Capture the method so the next run is cheaper and easier to check.

::: {.notes}

Git feels underrated to me here.
When AI makes it cheap to generate large changes, being able to isolate an experiment, inspect a diff, keep a known-good version, and recover becomes especially valuable.
Separate branches or worktrees let agents work independently; reviewing and merging their changes is still a coordination task.
The same habits apply to Markdown, configuration, prompts, and other text artifacts.

Being able to manipulate text with code extends this advantage beyond software development.
For example, extract every citation from a set of documents, apply a consistent terminology change, compare versions, or validate hundreds of records against a schema.
AI can help write the transformation; the script makes the operation explicit, repeatable, and testable.

These are plausible explanations for an engineering advantage, not individual mechanisms tested by the study on the next slide.
That study measured CS achievement, writing skill, and general cognitive ability; it did not separately test Git proficiency or text-processing skills.
